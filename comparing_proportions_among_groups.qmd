...
```{r}
#This compared the data of female/male and death
...
```
Null hypothesis, survivial does not depend on sex, is rejected because the p-value is less then 0.05.
...
fisher.test(table(heart_attacks[heart_attacks$AGE < 30, "SEX"], 
                  heart_attacks[heart_attacks$AGE <30, "DIED"]))
```
Because p-value is greater then 0.05, we fail to reject the null hypothesis. 
...
fisher.test(matrix(c(7, 1, 3, #spacing just for visuals
                     ...
                     9,1,7), nrow = 4, byrow = T))
```
Because the p-value is less then 0.05, we reject the null hypothesis.
